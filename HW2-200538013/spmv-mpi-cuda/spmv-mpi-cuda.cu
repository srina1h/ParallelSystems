...
__global__ void spmv_kernel(int *rows, int *cols, float *vals, float *x, float *y, int nonzeros)
{
...
int main(int argc, char **argv)
{
    ...
    int *d_local_rows, *d_local_cols;
    float *d_local_vals, *d_x, *d_local_y;

    cudaMalloc((void **)&d_local_rows, local_nonzeros * sizeof(int));
    cudaMalloc((void **)&d_local_cols, local_nonzeros * sizeof(int));
    cudaMalloc((void **)&d_local_vals, local_nonzeros * sizeof(float));
    cudaMalloc((void **)&d_x, global_num_cols * sizeof(float));
    cudaMalloc((void **)&d_local_y, rcount * sizeof(float));

    // Copy data to device
    cudaMemcpy(d_local_rows, local_rows, local_nonzeros * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_local_cols, local_cols, local_nonzeros * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_local_vals, local_vals, local_nonzeros * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_x, x, global_num_cols * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemset(d_local_y, 0, rcount * sizeof(float));
    ...
    int blocks_per_grid = (local_nonzeros + threads_per_block - 1) / threads_per_block;
    spmv_kernel<<<blocks_per_grid, threads_per_block>>>(d_local_rows, d_local_cols, d_local_vals, d_x, d_local_y, local_nonzeros);

    cudaDeviceSynchronize();

    // Copy result back to host
    cudaMemcpy(local_y, d_local_y, rcount * sizeof(float), cudaMemcpyDeviceToHost);
    ...
    cudaFree(d_local_cols);
    cudaFree(d_local_vals);
    cudaFree(d_x);
    cudaFree(d_local_y);

    if (rank == 0)
    {
        printf("Total time for full matrix multiplication: %f seconds\n", elapsed);
